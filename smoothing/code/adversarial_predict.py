...
def load_adversarial_data(threshold):
    if threshold == None:
        threshold = 'None'
    # load data
    with open(os.path.join('substitute_model_testing', 'l2_adversarial_output_threshold_%s.pkl' % threshold), 'rb') as f:
...
if __name__ == "__main__":
    ...
    images, labels, distances = load_adversarial_data(args.threshold)
    dataset = get_dataset(args.dataset, args.split)

    # adversarial_dataset = [None] * len(images)
    for i in range(len(dataset)):
        ...
        # (x, label) = dataset[i]
        (x, label) = (torch.FloatTensor(images[i]), dataset[i][1])
        x = x.cuda()
        before_time = time()
