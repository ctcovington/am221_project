...
class L2Adversary(object):
    ...
    def __call__(self, model, inputs, targets, to_numpy=True):
        ...
        targets_oh = torch.zeros(targets.size() + (num_classes,)).to(self.device)  # type: torch.FloatTensor
        ...
        pert_tanh = torch.zeros(inputs.size()).to(self.device)  # type: torch.FloatTensor
        if self.init_rand:
    ...
    def _optimize(self, model, optimizer, inputs_tanh_var, pert_tanh_var,
        ...
        batch_loss = batch_loss_var.data.item()  # type: float
